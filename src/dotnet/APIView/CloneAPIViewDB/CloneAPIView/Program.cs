    ...
    Container revisionsContainerNew, Container mappingsContainer, int? limit = null)
{
    ...
    var reviewsOldQuery = $"SELECT * FROM Reviews c Where Exists(Select Value r from r in c.Revisions where IS_DEFINED(r.Files[0].PackageName) AND r.Files[0].PackageName != \"\" AND NOT IS_NULL(r.Files[0].PackageName))";
